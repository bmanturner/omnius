...
#[cfg(feature = "api-key")]
use omnius_auth_core::{Scope, SubjectId, TenantId};
#[cfg(feature = "jwt")]
...
use serde::Serialize;
#[cfg(feature = "api-key")]
use serde::{Deserialize, ser::SerializeStruct as _};
use thiserror::Error;
...
#[cfg(feature = "api-key")]
use crate::map_json_rejection;
use crate::{ApiError, ProblemDetailsSchema, resolve_request_id};
...
/// Value-free classification returned by an injected bearer-token verifier.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum BearerTokenVerifyError {
    /// The credential is malformed, invalid, inactive, or otherwise rejected.
    Rejected,
    /// Authoritative token state could not be checked.
    Unavailable,
}

/// Application-supplied verification port for bearer token classes beyond JWT.
pub trait BearerTokenVerifier: Send + Sync {
    /// Verifies one bearer presentation into its canonical principal.
    fn verify<'a>(
        &'a self,
        token: &'a str,
    ) -> BoxFuture<'a, Result<Principal, BearerTokenVerifyError>>;
}
...
pub struct CanonicalPrincipalState {
    ...
    bearer_token_verifier: Option<Arc<dyn BearerTokenVerifier>>,
...
impl CanonicalPrincipalState {
    ...
    pub const fn new(
        pool: PostgresPool,
        session_config: SessionConfig,
        #[cfg(feature = "jwt")] jwt_verifier: Option<JwtVerifier>,
        #[cfg(feature = "api-key")] api_key_store: Option<ApiKeyStore>,
    ) -> Self {
        Self {
            pool,
            session_config,
    ...
    /// Enables an application-owned bearer-token class before JWT verification.
    #[must_use]
    pub fn with_bearer_token_verifier(mut self, verifier: Arc<dyn BearerTokenVerifier>) -> Self {
        self.bearer_token_verifier = Some(verifier);
        self
    }
...
async fn authenticate_authorization(
    ...
    if scheme.eq_ignore_ascii_case("bearer") {
        if let Some(verifier) = state.bearer_token_verifier.as_ref() {
            match verifier.verify(presentation).await {
                Ok(principal) => return Ok(principal),
                Err(BearerTokenVerifyError::Unavailable) => {
                    return Err(CredentialFailure::Unavailable);
                }
                Err(BearerTokenVerifyError::Rejected) => {}
            }
        }
        #[cfg(feature = "jwt")]
        {
...
pub(crate) async fn current_principal(
    ...
    let principal = PrincipalResponse::from_principal(principal)
        .map_err(|_| AuthenticationError::internal(request_id))?;
...
mod management {
    ...
    fn parse_issued_scopes(
        ...
        let mut scopes = raw
            .iter()
            .map(|value| {
                Scope::new(value).map_err(|_| ApiKeyHttpError::invalid_request(request_id))
            })
            .collect::<Result<Vec<_>, _>>()?;
    ...
    fn decode_cursor(
        ...
        let opaque = OpaqueCursor::try_from(raw)
            .map_err(|_| ApiKeyHttpError::invalid_request(request_id))?;
        let payload = codec
            .decode(&opaque)
            .map_err(|_| ApiKeyHttpError::invalid_request(request_id))?;
    ...
    const fn map_tenancy_error(error: TenancyStoreError, request_id: RequestId) -> ApiKeyHttpError {
        match error {
            ...
            | TenancyStoreError::MembershipNotFound => {
                ApiKeyHttpError::permission_denied(request_id)
            }
