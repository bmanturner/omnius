...
/// Redis session store with a feature-stable value codec.
///
/// The maintained provider serializes records through `MessagePack`. A session
...
#[derive(Clone, Debug)]
pub struct FeatureStableRedisStore {
    inner: RedisStore<Client>,
}

impl FeatureStableRedisStore {
    /// Creates an adapter around the maintained Redis provider.
    #[must_use]
    pub fn new(client: Client) -> Self {
        Self {
            inner: RedisStore::new(client),
        }
    }
}

#[async_trait]
impl SessionStore for FeatureStableRedisStore {
