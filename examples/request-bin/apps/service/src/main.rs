...
#[derive(Debug, Parser)]
#[command(name = "request-bin", version, about = "Generated Omnius service")]
struct Cli {
    #[command(subcommand)]
    command: Command,
}
...
impl AppConfig {
    fn validate_composition(&self, environment: EnvironmentArg) -> Result<(), StartupError> {
        if self.telemetry.service != "request-bin"
...
mod tests {
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        let filename = format!("request-bin-contracts-{}.json", std::process::id());
