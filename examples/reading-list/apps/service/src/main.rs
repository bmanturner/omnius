...
#[derive(Debug, Parser)]
#[command(name = "reading-list", version, about = "Generated Omnius service")]
struct Cli {
    #[command(subcommand)]
    command: Command,
}
...
impl AppConfig {
    fn validate_composition(&self, environment: EnvironmentArg) -> Result<(), StartupError> {
        if self.telemetry.service != "reading-list"
...
mod tests {
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        let filename = format!("reading-list-contracts-{}.json", std::process::id());
