...
function createCrossTabPort(): CrossTabAuthSignalPort {
  ...
  const channel = new BroadcastChannel("reading-list-auth-session");
  return Object.freeze({
    publish(value: unknown): void {
...
export function createBrowserSessionAuthManager(
  ...
  const readingItemsKey = serviceQueries.getListReadingItemsQueryKey();
  ...
  return createSessionAuthManager<LoginCredentials, never>({
    ...
    identityLifecycle: createQueryIdentityTransitionLifecycle({
      queryClient,
      localState: [
        {
          async resetForIdentityTransition(): Promise<void> {
            await queryClient.cancelQueries({ queryKey: readingItemsKey });
            queryClient.removeQueries({ queryKey: readingItemsKey });
          },
        },
      ],
    }),
