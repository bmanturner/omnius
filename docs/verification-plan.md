...
`cargo run --locked --package xtask --`. Every direct dependency-consuming
Cargo command below also names `--locked`.
...
| Command or scenario | With the immutable installed CLI, create a nonexistent disposable project using `cargo service new matrix-minimal --profile minimal --path "$PROJECT_PATH"`. Run `cargo service add openapi --dry-run --project "$PROJECT_PATH"`; after reviewing the sealed plan, apply it without `--dry-run`. Run `cargo service doctor --project "$PROJECT_PATH"` and `cargo service diff --project "$PROJECT_PATH"`. From the generated root, run `cargo test --workspace --locked` and invoke the generated binary's `profile-info` subcommand with locked dependency resolution. Back in the Omnius source repository, run `cargo xtask contracts check`. |
| Prerequisites | The clean revision-bound `cargo-service`; a nonexistent disposable destination; `openapi` and its `http`/`validation` requirements from the checked-in runtime catalog; no edits worth preserving. Persistence requires a disposable database, application migrations in the reserved range, compatibility bounds, and a cleanup owner. |
| Expected observation | The sealed plan and `profile-info` show OpenAPI and its runtime closure without idempotency. Doctor and diff are clean, the generated workspace test passes, and `Cargo.lock` remains byte-identical across inspection/build/test. The fresh application extension is empty, so `/example` and `/reference-records` are absent until application-owned contributions add routes. |
| Failure-path check | Run `cargo service add rate-limit-redis --dry-run --project "$PROJECT_PATH"`; the selected `rate-limit-local` provider must make the plan fail without changing the project. Attempting to add a tooling module must also fail before resolution or mutation. |
| Current result | not run |
...
| Command or scenario | Run `cargo xtask ai verify`, `cargo test --locked --tests -p omnius-mcp-server-core -p omnius-mcp-transport-http -p omnius-mcp-tools -p omnius-mcp-resources -p omnius-mcp-prompts -p omnius-mcp-auth-oauth`, and `cargo run --locked -p omnius-mcp-conformance --bin mcp-conformance -- synthetic`. `cargo run --locked -p omnius-mcp-conformance --bin mcp-conformance -- official-plan-http http://127.0.0.1:9010/mcp` can inspect the pinned official HTTP plan, but there is no first-party process at that endpoint. |
| Prerequisites | Repository fixtures for the commands above. A runtime run additionally requires a concrete capability registry, deny-by-default authorizer, identity and tenant, authenticated `/mcp`, negotiated extensions, and persistence/backplane where durable behavior is claimed. MRTR and Tasks migrations are checked in and embedded by the common migrator, but an application that composes those capabilities must apply them to its disposable database and prove the applied version. |
...
| Command or scenario | Only in an explicitly disposable development environment, run `cargo run --locked --bin omnius-api-server -- migrate --config config/reference.toml --environment development`, then `cargo run --locked --bin omnius-api-server -- migration-status --config config/reference.toml --environment development`. Start the same target with `cargo run --locked --bin omnius-api-server -- server --config config/reference.toml --environment development` and request `curl --fail --silent --show-error http://127.0.0.1:8080/ready`. Run `scripts/recovery/rehearse-local` for synthetic backup/restore/rollback evidence, then `cargo xtask contracts diff --against fixtures/contract-compatibility/baseline`. |
| Prerequisites | An isolated disposable development PostgreSQL database with `POSTGRES_URL` pointing only to it; test-only configuration substitutions and credentials; explicit rehearsal authorization; schema range `2026082301` through `2026082809`; Docker for the local synthetic recovery script; free port `8080`; recorded baseline fixture and cleanup owner. No production credential, endpoint, or database route may be present. |
| Expected observation | Migration status reports target version `2026082809`, the concrete reference API reports ready, the local recovery artifact meets its synthetic fingerprint and timing checks, configuration failures omit secret values, and the contract diff is compatible. Redaction is not rotation. Deployment and secret rotation require separate platform-owned test-environment rehearsals with revision-bound, redacted evidence; without those owners and environments, those parts remain blocked. |
...
## Profile evidence contract and follow-up examples handoff

`OMNIUS_RELEASE_REVISION="$REV" cargo xtask profiles generate-verify --jobs 1 --automated-evidence-only`, where `REV` is a remotely reachable full commit SHA, writes the sole machine handoff at `target/profile-matrix/report.json` (schema 5). Generated thin consumers resolve their framework dependency from that exact revision; a placeholder or local-only commit is invalid. Profiles build sequentially; after each row records its evidence, the task removes that profile's Cargo cache and retains only the generated binary plus report artifacts. The report contains exactly one row for each of the 23 catalog profiles and the five families `base`, `web`, `ai`, `mcp`, and `ai_mcp`. Required check IDs are `composition-manifest`, `migration-policy`, `startup-readiness`, `registered-routes-tasks-health`, `representative-workflow`, `negative-workflow`, `dependency-outage`, `bounded-shutdown`, `runtime-contract-parity`, and `build-cache-cleanup`. Runtime-contract parity compares generated contracts with mounted route and capability IDs; a generated file alone is not runtime proof. The `assembled` state additionally requires a compiled generated process, exact registrar completion, no synthetic application fixture, no unresolved application contribution, and successful runtime checks. `compiled` proves only the generated package built. `generated` proves only render and generator metadata. `selected` proves only catalog resolution.

The follow-up examples task must consume the report rather than infer readiness:

- **Ready without application contributions:** `minimal` (no external dependency), plus `api` and `web-sdk-only` after disposable PostgreSQL is migrated. These are candidates only until their untouched generated-root rows retain all required observations; this document does not promote them.
- **Application contributions required:** `authenticated-api`, `oauth-provider`, and `web` require the authenticated/OAuth application runtime; `worker`, `saas`, and `saas-pgmq` require the exact job/outbox/scheduler/inbox/feature/admin/webhook contribution names in their report rows; `realtime`, `realtime-durable`, `realtime-web`, and `saas-web` require fanout authorization, identity revalidation, and event handling; `mcp-http`, and `mcp-enterprise` require the canonical capability registry plus their named context/auth/subscription/task/enterprise/Apps ports; every AI row requires its named tool, audit, media, evaluation, job, or MCP ports; both full-reference rows require search, billing, GraphQL, gRPC, privacy, consent, and moderation policy ports. `full-reference-web` inherits those blockers.
- **Disposable dependency topology:** PostgreSQL for API/auth/migration/task/conversation/ledger roots; Redis for Apalis, cache, ephemeral realtime, and Redis MCP subscriptions; PostgreSQL with prior PGMQ provisioning for `saas-pgmq`; NATS JetStream for durable event roots; object storage plus scanner, SMTP, and webhook fixtures where selected. Start only services listed in each row's `resolved_services`; an unavailable service records blocked process evidence and keeps the row unassembled.
- **Commands/workflows to retain:** render the row, execute its reported Cargo package with the `profile-info` subcommand, and retain the composition manifest; run `migration-status` and controlled `migrate` only against disposable data; start the row's reported `executable_command`; probe `/ready`, `/version`, the representative positive route, an absent or unauthorized negative route, dependency loss, and SIGTERM drain. Generated MCP/AI rows that stop at application-owned requirements remain blocked; do not invent discover/invoke/provider evidence. Independently run the dedicated `apps/mcp-server` metadata, bearer, discover/list/invoke/method-not-found/drain flows and retain them only as reference-app evidence. Retain `cargo xtask contracts check` parity output and report artifact digests.
- **Non-negotiable blockers:** `llm-embeddings` remains specified-only until an authoritative operation request schema and provider port exist. `mcp-enterprise` and `full-reference-ai` remain unassembled until enterprise identity/link/replay/live-state/consent/audit ports, Apps lifecycle/storage/action-lease ports, and a genuine JetStream subscription backplane exist; Core NATS fanout is insufficient. Full-reference profiles additionally remain blocked by the application-owned product/privacy ports above. Library/router tests, generated artifacts, deterministic LLM doubles, and synthetic application fixtures never satisfy default assembly.