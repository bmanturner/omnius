...
`cargo xtask profiles generate-verify --jobs 1 --automated-evidence-only` writes the sole machine handoff at `target/profile-matrix/report.json` (schema 5). Profiles build sequentially; after each row records its evidence, the task removes that profile's Cargo cache and retains only the generated binary plus report artifacts. The report contains exactly one row for each of the 23 catalog profiles and the five families `base`, `web`, `ai`, `mcp`, and `ai_mcp`. Required check IDs are `composition-manifest`, `migration-policy`, `startup-readiness`, `registered-routes-tasks-health`, `representative-workflow`, `negative-workflow`, `dependency-outage`, `bounded-shutdown`, `runtime-contract-parity`, and `build-cache-cleanup`. Runtime-contract parity compares generated OpenAPI operation paths/IDs and compiled capability/transport declarations with registrations admitted by the static registrar graph. A skipped required check is failed evidence. An installed application fixture must set `application_fixture.synthetic=true` and `used_for_classification=false`.
